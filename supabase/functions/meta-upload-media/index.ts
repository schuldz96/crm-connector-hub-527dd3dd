 ...
 *   - file: o arquivo (com mime type correto)
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[meta-upload-media] Uploading: ${file.name} (${file.type}, ${file.size} bytes) to ${phoneNumberId}`);

    // Forward to Meta API — preserve file name and type exactly as received
    ...
    console.log(`[meta-upload-media] Result: ${JSON.stringify(data)}`);
    ...
    console.error(`[meta-upload-media] Error: ${e.message}`);
