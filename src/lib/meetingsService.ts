...
/**
 * Ensure all internal-domain participants across meetings are registered users.
...
export async function ensureAppmaxParticipantsRegistered(meetings: DbMeeting[]): Promise<string[]> {
  ...
  for (const m of meetings) {
    for (const p of m.participantes) {
      if (p.email && p.email.toLowerCase().endsWith(`@${domain}`)) {
        internalEmails.add(normalizeEmail(p.email));
      }
    }
    if (m.vendedor_email && m.vendedor_email.toLowerCase().endsWith(`@${domain}`)) {
      internalEmails.add(normalizeEmail(m.vendedor_email));
    }
  }
  ...
  const missing = Array.from(internalEmails).filter(e => !existingSet.has(e));
